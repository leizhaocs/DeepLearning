...
__global__ void backward_softmax_kernel(float *backward_input, float *forward_output, float *backward_output, int total_size)
{
    int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
    if (i < total_size)
    {
...
void backward_softmax_gpu(Tensor<float> *backward_input, Tensor<float> *forward_output, Tensor<float> *backward_output)
{
    ...
    backward_softmax_kernel<<<GRID(total_size), BLOCK>>>(backward_input_ptr, forward_output_ptr, backward_output_ptr, total_size);
    check_cuda_error();
}
